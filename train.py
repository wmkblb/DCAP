...
import trainers.dcap_teacher
import trainers.caspl_cocoop
import trainers.caspl_coop
import trainers.dcap
import trainers.caspl_promptsrc
import os
...
def extend_cfg(cfg):
    ...
    cfg.KD = CN()
    cfg.KD.FIRST_TRAIN = False         #use the the first stage
    cfg.KD.SECOND_TRAIN = False        #use the the second stage
    cfg.KD.BACKBONE_NAME = "ViT-L/14"
    cfg.KD.TEMPERATURE = 1
    cfg.KD.IS_TEST = False
    cfg.KD.N_CTX_VISION= 8
    cfg.KD.N_CTX_TEXT= 8
    cfg.KD.CTX_INIT= "a photo of a"
    cfg.KD.PREC= "fp16"
    cfg.KD.PROMPT_DEPTH_VISION= 12
    cfg.KD.PROMPT_DEPTH_TEXT= 12
    cfg.KD.RESULT_PATH = "results_path"
    cfg.KD.DIR_NAME = "dirname&1"
    cfg.KD.SECOND_PARAMETER_USE = False #update VPT_shallow_FIRST and VPT_shallow in the second stage
    cfg.KD.USE_IVLP = False             #the first stage
    cfg.KD.USE_DATASET_LIST = 0         #use the test set as train set
...
def main(args):
    ...
    if cfg.KD.SECOND_TRAIN != False:
        new_name = "DCAP"
        if not args.eval_only:
            print_info("Now here is loading the model of teacher")
            print(args.model_dir)
            trainer.load_model_one_stage(new_name, args.model_dir, epoch=args.load_epoch)
